...
describe('Admin Status Change E2E Tests', () => {
  ...
  async function loginAsAdmin() {
    ...
    console.log('Navigating to login page via client-side routing...');
  ...
  test('should successfully change report status via dropdown', async () => {
    ...
    const statusDropdown = await page.waitForSelector('button[class*="rounded-full"]', { timeout: 5000 });
    ...
    await page.waitForSelector('div[class*="absolute"]', { timeout: 3000 });

    // Select a different status (try to find "Confirmed by NJDSC" or any other option)
    console.log('Selecting new status...');
    const statusOptions = await page.$$('button[class*="w-full text-left"]');
    ...
    if (consoleErrors.length > 0) {
      console.log('Errors found:', JSON.stringify(consoleErrors, null, 2));
    }
